...
import rs.ems.data.flux.config.KafkaProperties;

class TestAdminKafkaResourceIT {
    ...
    void setup() {
        ...
        TestAdminKafkaResource kafkaResource = new TestAdminKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/test-admin-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/test-admin-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
